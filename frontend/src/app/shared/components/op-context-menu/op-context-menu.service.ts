...
export class OPContextMenuService {
   ...
   */
  public show(menu:OpContextMenuHandler, event:JQuery.TriggeredEvent|Event, component:any = OPContextMenuComponent) {
    this.close();
  ...
  public reposition(event:JQuery.TriggeredEvent|Event) {
    if (!this.active) {
      return;
    }
